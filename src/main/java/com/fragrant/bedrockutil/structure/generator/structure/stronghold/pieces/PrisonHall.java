...
public class PrisonHall extends StrongholdPiece { //Called SHPrisonHall in mojmap

	public PrisonHall(int pieceId, ChunkRandom rand, BoundingBox boundingBox, Direction facing) {
		super(pieceId);
		this.facing = facing;
	...
	public static PrisonHall createPiece(List<StrongholdPiece> pieces, ChunkRandom rand, int x, int y, int z, Direction facing, int pieceId) {
		BoundingBox box = BoundingBox.rotated(x, y, z, -1, -1, 0, 9, 5, 11, facing.getRotation());
		return StrongholdPiece.isHighEnough(box) && StrongholdPiece.findCollisionPiece(pieces, box) == null ? new PrisonHall(pieceId, rand, box, facing) : null;
	}
